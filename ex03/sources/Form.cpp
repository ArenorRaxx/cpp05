...
/*   Updated: 2023/02/11 19:57:52 by nikoraxx         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
Form::Form(void): _name("?"), _target("?"), _auth_grade(150), _exec_grade(150), _is_signed(false)
{
	// std::cout << "Form Default constructor.\n";
}

Form::Form(Form const &to_copy): _name(to_copy._name), _target(to_copy._target)
{
	// std::cout << "Form Copy constructor.\n";
	*this = to_copy;
}

Form::Form(std::string const &name, std::string const &target, int auth_grade, int exec_grade): _name(name), _target(target), _is_signed(false)
{
...
const char* Form::UnapprovedFormException::what(void) const throw()
{
	return ("Executing an unapproved form?! Are you crazy! What if it contains a typo?\n");
}
